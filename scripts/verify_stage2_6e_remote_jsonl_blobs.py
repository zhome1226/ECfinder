"""Verify Stage 2.6e JSONL blobs from HEAD, fresh clone, and GitHub raw URLs."""
...
REPORT = ROOT / "reports" / "stage2_6e_commit_blob_jsonl_verification.md"
REPO_RAW = "https://raw.githubusercontent.com/zhome1226/ECfinder"
TARGETS = [
    "data/batches/stage2_6e_streaming_screening_decisions.jsonl",
    "data/batches/stage2_6e_streaming_chunk_screening.jsonl",
    "data/batches/stage2_6e_streaming_candidate_records.jsonl",
    "data/batches/stage2_6e_streaming_reviewed_records.jsonl",
    "data/batches/stage2_6e_streaming_validated_records.jsonl",
    "data/batches/stage2_6e_streaming_manual_review_records.jsonl",
    "data/batches/stage2_6e_streaming_rejected_records.jsonl",
    "data/batches/stage2_6e_streaming_auxiliary_records.jsonl",
    "data/state/stage2_6e_streaming_source_status.jsonl",
    "data/state/stage2_6e_streaming_events.jsonl",
]
...
def fetch_raw(commit: str, target: str) -> str:
    ...
    raise RuntimeError(f"failed to fetch GitHub raw blob after retries: {url}: {last_error}")


def main() -> int:
    commit = run(["git", "rev-parse", "HEAD"])
    tmp_clone = TMP_ROOT / f"ecfinder_stage2_6e_jsonl_remote_verify_{os.getpid()}_{int(time.time())}"
    try:
        ...
        all_match = all(row["all_sha256_match"] for row in records)
        lines = [
            ...
            f"all_sha256_match = {str(all_match).lower()}",
            "",
        ...
        for row in records:
            ...
            lines.append(
                ...
                f"{row['github_raw']['physical_lines']} | `{row['github_raw']['sha256']}` | "
                f"{str(row['all_sha256_match']).lower()} | strict_jsonl_ok | `{first}` |"
            )
        REPORT.write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")
        print(json.dumps({"commit": commit, "targets": len(TARGETS), "all_sha256_match": all_match}, ensure_ascii=False, sort_keys=True))
